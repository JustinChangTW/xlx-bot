...
這是一個使用 Flask + LINE Messaging API、多模型 provider、模組化知識庫與受控 sidecar/OpenClaw 骨架的小龍蝦聊天機器人。
...
- **受控 OpenClaw 能力**：任務型請求先做風險分級，低風險可回答，中風險進 `pending review`，高風險禁止。
...
- `knowledge/` 採用一個主題一個檔的設計，回答流程會把它視為唯一正式社團事實來源。
...
- 官網抓取能力存在於 provider 層，但預設關閉；若未明確啟用 `OFFICIAL_SITE_RETRIEVAL_ENABLED=true`，正式回答只依賴本地知識。
...
7. 若請求屬於中風險任務（例如文件草稿、sidecar 建議），系統回覆 `pending review`；若屬於高風險（改程式、deploy、寫正式 knowledge），系統直接禁止。
8. 若有足夠知識，再依 route 選擇可用 provider chain，例如 Groq、xAI、GitHub Models、Gemini、Ollama。
...
- 風險分級：
  - `low`：知識查詢、回答、learning/troubleshooting 記錄
...
- 目前正式回答仍以檔案載入與規則式切用為主
- 尚未完成 Chroma / FAISS 向量索引
- 後續若加入 RAG，仍必須遵守：只檢索 `knowledge/`、保留來源、結果不足就拒答
...
- `OFFICIAL_SITE_RETRIEVAL_ENABLED` （可選，預設 `false`，是否允許用官網抓取補充回答；若要嚴格 knowledge-only，請維持 `false`）
...
- bot 亂答：先確認 `OFFICIAL_SITE_RETRIEVAL_ENABLED` 是否被打開，以及 `knowledge/` 是否有對應資料

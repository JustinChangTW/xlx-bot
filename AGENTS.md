...
- 先依據本地知識回答
- 本地無資料時，透過 OpenClaw 查詢已核可官方來源
- 本地與官方查核都無資料時，明確說資料不足
- 不得自造人名、活動、時間、幹部、公告
...
2. 判斷缺的是內容、格式、還是檢索方式
3. 優先補強結構、回答規則與 OpenClaw 官方查核路徑
4. 缺少真實資料時，用明確 TODO / 待補資料標記
5. OpenClaw 查到的新答案先寫入 pending review，不直接寫入正式 `knowledge/`
6. 不可自行補造「現任幹部」「最新公告」「課程活動」等真實資訊
...
3. 檢查是否有根據問題類型分流
4. 檢查是否有「本地不足就 OpenClaw 查核；仍不足才拒答 / 資料不足」機制
5. 回答設計要先短答再補充
...
1. 知識切段要保留標題與來源
2. 回答時只用最相關片段
3. 若本地檢索結果不足，先透過 OpenClaw 查詢已核可官方來源；仍不足不可硬答
4. 優先做最小可行版本
